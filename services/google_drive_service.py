"""
Google Drive Service – Upload defensiv cu logging detaliat.
Fiecare pas este verificat și logat individual.
"""
...
# Forțăm recitirea .env la import, indiferent de ordinea importurilor
...
# ═══════════════════════════════════════════════════════════════════════
# PASUL 0 — Citirea configurației OAuth2 din .env
# ═══════════════════════════════════════════════════════════════════════

def _load_config():
    """Citește variabilele din .env și verifică că sunt completate."""
...
# ═══════════════════════════════════════════════════════════════════════
# PASUL 1 — Construirea serviciului Google Drive cu OAuth2
# ═══════════════════════════════════════════════════════════════════════

def _get_drive_service():
    """Construiește și returnează un serviciu Google Drive autentificat ca TINE."""
...
# ═══════════════════════════════════════════════════════════════════════
# PASUL 2 — Verificarea accesului la folder
# ═══════════════════════════════════════════════════════════════════════

def verify_folder_access(service, folder_id):
    """Verifică dacă Service Account-ul are acces la folderul specificat."""
...
# ═══════════════════════════════════════════════════════════════════════
# PASUL 3 — Upload-ul propriu-zis
# ═══════════════════════════════════════════════════════════════════════

def upload_to_drive(file_path, file_name):
    """
    Uploadează un fișier pe Google Drive cu verificări complete.

    Args:
        file_path: Calea absolută către fișierul de uploadat.
        file_name: Numele fișierului pe Drive.

    Returns:
        tuple: (file_id, web_content_link) sau (None, None) în caz de eroare.
    """
    print("", flush=True)
    print("=" * 70, flush=True)
    ...
    # ── Verificare 1: Fișierul local există? ──
    ...
    # ── Verificare 2: Construire serviciu Drive ──
    try:
    ...
    # ── Verificare 6: Setare permisiuni publice ──
    try:
...
def get_download_url(file_id):
    """Returns the webContentLink for a given file_id."""
    try:
        service = _get_drive_service()
...
def delete_from_drive_by_link(web_content_link):
    """
    Sterge un fisier din Google Drive extragand ID-ul din linkul public.
    Returneaza True la succes sau daca link-ul/fisierul lipseste.
    """
...
def delete_file_from_drive(file_id):
    """
    Șterge un fișier din Google Drive după file_id.
    Nu dă eroare dacă fișierul nu există (404).
    Returnează (True, None) dacă s-a șters cu succes, (False, eroare) altfel.
    """
    if not file_id:
    ...
    try:
        ...
        if status == 404:
            ...
            return True, None  # Tratăm ca succes
        elif status == 403:
...
# ═══════════════════════════════════════════════════════════════════════
# GHID DE DEBUGGING (5 cauze frecvente)
# ═══════════════════════════════════════════════════════════════════════
#
# Dacă primești erori, verifică aceste 5 lucruri, în ordine:
#
# 1. FIȘIERUL JSON LIPSĂ SAU INVALID
#    → Verifică că fișierul `.json` descărcat de la Service Account
#      se află în folderul backend-ului și că numele din .env e corect.
#    → Google Cloud Console → IAM & Admin → Service Accounts → Keys
#
# 2. VARIABILELE DIN .ENV NU SUNT CITITE
#    → După ce modifici .env, TREBUIE să repornești serverul Flask!
#    → Oprește cu Ctrl+C și pornește din nou cu: python app.py
#
# 3. SERVICE ACCOUNT-UL NU ARE ACCES LA FOLDER
#    → Mergi în Google Drive → Click dreapta pe folder → Share
#    → Adaugă email-ul Service Account-ului (cel din JSON, câmpul
#      "client_email") cu rol de EDITOR.
#    → Google Cloud Console: IAM & Admin → Service Accounts → vezi email-ul
#
# 4. GOOGLE DRIVE API NU ESTE ACTIVAT
#    → Google Cloud Console → APIs & Services → Enabled APIs
#    → Caută "Google Drive API". Dacă nu e acolo, apasă "Enable".
#    → Link direct: https://console.cloud.google.com/apis/library/drive.googleapis.com
#
# 5. FOLDER ID GREȘIT
#    → Deschide folderul în browser. URL-ul arată așa:
#      https://drive.google.com/drive/folders/XXXXXXXXXXXXXXX
#    → Copiază doar partea de după /folders/ (acel XXXXXXXX) în .env
#
# ═══════════════════════════════════════════════════════════════════════
